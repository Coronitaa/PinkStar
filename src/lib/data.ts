...
const calculateSearchScore = (resource: Resource, query: string): number => {
  ...
  if (resource.name.toLowerCase().includes(lowerQuery)) {
    ...
    if (resource.name.toLowerCase().startsWith(lowerQuery)) score += 5; // Bonus for prefix match in name
    if (resource.name.toLowerCase() === lowerQuery) score += 10; // Strong bonus for exact name match
  }
  if (resource.description.toLowerCase().includes(lowerQuery)) {
    score += 3; // Lower score for short description
  }
  if (resource.detailedDescription.toLowerCase().includes(lowerQuery)) {
    // Score based on frequency or prominence in detailed description could be added here
    // For now, a simple check:
    const overviewMentions = (resource.detailedDescription.toLowerCase().match(new RegExp(lowerQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    score += overviewMentions * 1; // 1 point per mention in detailed description
  }
...
export const getResources = async (params: GetResourcesParams): Promise<PaginatedResourcesResponse> => {
  ...
  switch (sortBy) {
    case 'relevance':
      // If there's an active search query, relevance is primarily driven by searchScore.
      // Otherwise, a default relevance (e.g., by downloads or recency) can be applied.
      if (params?.searchQuery && params.searchQuery.length > 0) {
        filteredResources.sort((a, b) => (b.searchScore || 0) - (a.searchScore || 0));
      } else {
        // Default "relevance" without search query: combination of downloads and recency.
