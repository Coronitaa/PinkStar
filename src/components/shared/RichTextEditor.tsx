...
const ImageCarouselModal = ({ isOpen, onOpenChange, initialImages = [], initialAspectRatio = '16/9', initialAutoplayInterval = 5000, onSave }: { isOpen: boolean, onOpenChange: (open: boolean) => void, initialImages?: string[], initialAspectRatio?: string, initialAutoplayInterval?: number, onSave: (config: { images: string[], aspectRatio: string, autoplayInterval: number }) => void }) => {
  ...
  useEffect(() => { if (isOpen) { setImages(JSON.parse(initialImagesStr)); setAspectRatio(initialAspectRatio); setAutoplaySeconds(initialAutoplayInterval === 999999999 ? 0 : initialAutoplayInterval / 1000); setAutoplayInterval(initialAutoplayInterval); setIsImporting(false); setImportText(''); } }, [isOpen, initialImagesStr, initialAspectRatio, initialAutoplayInterval]);
...
const ImageCarouselComponent = (props: NodeViewProps) => {
  ...
  const handleSaveCarousel = (c: any) => updateAttributes(c);
...
const Toolbar = ({ editor }: { editor: Editor | null }) => {
  ...
  const handleSaveCarousel = (c: any) => editor?.chain().focus().setImageCarousel(c).run();
