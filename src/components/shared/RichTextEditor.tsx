...
// Removed individual language imports as they are now in @/lib/lowlight
import {
...
const CodeBlockComponent = (props: NodeViewProps) => {
  ...
  // We don't need to fetch the theme here anymore, as the styles are injected globally via CodeHighlightStyle.tsx
  // matching .tiptap-code-block selector.
  ...
  return (
    <NodeViewWrapper className="not-prose my-4 relative group/code-block">
      <div
        className="relative border border-border rounded-lg overflow-hidden flex flex-col"
      >
        <div className="flex items-center justify-between bg-card-foreground/5 px-2 py-1.5 border-b border-border text-xs shrink-0 select-none">
          <div className="flex items-center gap-2 flex-grow min-w-0">
            ...
            <input
              className="bg-transparent text-muted-foreground outline-none placeholder:text-muted-foreground/70 text-xs w-full min-w-0"
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            >
              <SelectTrigger className="h-6 text-xs w-[100px] border-none bg-transparent hover:bg-muted/50 focus:ring-0 focus:ring-offset-0 px-2">
            ...
            <Button variant="ghost" size="icon" className="h-6 w-6 hover:bg-muted/50" onClick={handleCopy}>
              {isCopied ? <Check className="w-3.5 h-3.5 text-green-500" /> : <ClipboardCopy className="w-3.5 h-3.5 text-muted-foreground" />}
            </Button>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="h-6 w-6 hover:bg-muted/50" title="Block settings">
            ...
            <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive/70 hover:text-destructive hover:bg-destructive/10" onClick={() => editor.chain().focus().deleteNode('customCodeBlock').run()} title="Delete block">
        ...
        <pre
          ref={preRef}
          className="tiptap-code-block m-0 p-4 !bg-transparent"
          style={{
            maxHeight: node.attrs.maxHeight,
            overflowY: 'auto',
          }}
        >
    ...
    return [
      {
        ...
        priority: 100, // Enforce priority to match the wrapper div
  ...
  renderHTML({ node, HTMLAttributes }) {
    return ['div',
      mergeAttributes(HTMLAttributes, {
        ...
        // Ensure attributes are on the root div for proper serialization/deserialization
      }),
      ['pre', {}, ['code', 0]]
    ]
  },
...
export const RichTextEditor = ({ initialContent, onChange, readonly }: RichTextEditorProps) => {
  ...
  // Generate scoped CSS for this editor instance
  const codeStyles = useMemo(() => {
    return theme ? generateHighlightCss(theme.styles, `#${editorId} .tiptap-code-block`) : '';
  }, [theme, editorId]);
  ...
  return (
    <div id={editorId} className="w-full rounded-md border border-input bg-background ring-offset-background focus-within:outline-none focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
      {codeStyles && <style>{codeStyles}</style>}
      {editor && (
        <BubbleMenu
          editor={editor}
