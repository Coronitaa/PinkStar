...
const RatingDisplay: React.FC<{ rating?: number, compact?: boolean }> = ({ rating, compact = false }) => {
  ...
  return (
    <div className={cn("flex items-center", compact ? "gap-0.5" : "gap-0.5")}>
      ...
      {halfStar && <Star key="half" className={cn(starSize, "text-amber-400 fill-amber-200")} />} {/* Simplified half-star look */}
    ...
    >
      <Card 
        className={cn(
          ...
          compact ? "text-xs" : "" // Global text size for compact card
        )}
      >
      ...
      {/* Hover Detail Overlay - Size is NOT affected by 'compact' prop */}
      <Link href={`/resources/${resource.slug}`} passHref legacyBehavior>
        <a 
          className={cn(
            "detail-overlay absolute inset-0 bg-card/95 backdrop-blur-md p-4 rounded-lg shadow-2xl border border-primary/50", // Standard padding for overlay
            "flex flex-col transition-all duration-300 ease-in-out z-10", // Ensure it's above base card
            "opacity-0 scale-95 pointer-events-none",
            isHovering && !compact && "opacity-100 scale-100 pointer-events-auto", // Only show overlay if NOT compact
            isHovering && compact && "opacity-100 scale-100 pointer-events-auto"  // Re-enable for compact hover
          )}
          onClick={(e) => { if (e.target !== e.currentTarget) e.stopPropagation(); }} // Allow clicks on overlay content without navigating if not on overlay itself
        >
          {/* Image Carousel in Overlay */}
          ...
          <div className="mt-auto pt-2 border-t border-border/20 flex items-center gap-2">
              ...
              >
                  <Button asChild variant="outline" size={latestFile ? "icon" : "sm"} className={cn("button-outline-glow text-xs h-auto", latestFile ? "p-2" : "w-full py-1.5")}>
                     <a>
