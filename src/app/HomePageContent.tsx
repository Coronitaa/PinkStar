...
import type { GameWithDetails } from './page'; // Assuming type is exported from page.tsx
import { GameCard } from '@/components/game/GameCard';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Search, ListFilter, Gamepad2 } from 'lucide-react';

type SortOption = 'popularity' | 'name_asc' | 'name_desc' | 'default';
...
const calculateGameSearchScore = (game: GameWithDetails, query: string): number => {
  ...
  if (game.name.toLowerCase().includes(lowerQuery)) {
    ...
    if (game.name.toLowerCase() === lowerQuery) score += 10; // Boost for exact match
  }
...
export function HomePageContent({ initialGames }: { initialGames: GameWithDetails[] }) {
  ...
  const filteredAndSortedGames = useMemo(() => {
    ...
    if (debouncedSearchQuery.trim()) {
      gamesToProcess = gamesToProcess
        ...
        .sort((a, b) => (b.searchScore || 0) - (a.searchScore || 0)); // Sort by score
    }
    ...
    switch (sortBy) {
      ...
      case 'name_desc':
        ...
        break;
      case 'default':
         // If no search and default, keep original order or apply a default logic if any
        if (!debouncedSearchQuery.trim()) {
            // Example: Default sort by popularity if no search and 'default' sort
            gamesToProcess.sort((a, b) => b.stats.totalDownloads - a.stats.totalDownloads);
        }
        // If there was a search, it's already sorted by relevance
        break;
    }
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="mb-8 p-4 border rounded-lg bg-card shadow-md sticky top-16 z-40 backdrop-blur-sm bg-background/80"> {/* Make controls sticky */}
