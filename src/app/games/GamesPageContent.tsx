...
import type { GameWithDetails } from './page'; // Adjusted import path if page.tsx is in the same folder
...
const calculateGameSearchScore = (game: GameWithDetails, query: string): number => {
  ...
  if (game.name.toLowerCase().includes(lowerQuery)) {
    score += 10;
    if (game.name.toLowerCase().startsWith(lowerQuery)) score += 5;
    if (game.name.toLowerCase() === lowerQuery) score += 10;
  }
...
// Renamed from HomePageContent to GamesPageContent
export function GamesPageContent({ initialGames }: { initialGames: GameWithDetails[] }) {
